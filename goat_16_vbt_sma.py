"""
GOATv2 Numba Backtest — 0.5R BE + 200 SMA Filter
Long only above SMA200, Short only below SMA200.
Parametric study: with/without SMA filter + BE levels.
...
def calculate_sma(raw_close, period=200):
    """Simple Moving Average on raw close prices."""
    sma = np.full(len(raw_close), np.nan, dtype=np.float64)
    if len(raw_close) < period:
        return sma
    # Cumulative sum approach for speed
...
def precompute_all(df_raw):
    """Compute HA, patterns, pivots, sparse tables, SMA ONCE."""
...
def run_backtest(pre, rr_ratio=3, be_trigger_r=None, use_sma_filter=False, warmup=300):
    t0 = time_module.perf_counter()

    be_label = f"{be_trigger_r}R BE" if be_trigger_r else "NO BE"
    sma_label = "SMA200" if use_sma_filter else "NO FILTER"
    print(f"\n{'='*60}")
    print(f"BACKTEST: {be_label} | {sma_label}")
    print(f"{'='*60}")
...
def print_trade_table(trades, label=""):
    ...
    for i, t in enumerate(trades):
        ...
        print(f"{i+1:>3} {t['side']:<5} {t['case']:<4} {m:<3} "
              ...
              f"{t.get('duration_bars',''):>4} {sma_rel:>5} "
...
def export_csv(trades, filename):
    ...
    for i, t in enumerate(trades):
        rows.append({
            ...
            "sma200": round(t.get("sma_val", 0), 2),
            "price_at_entry": round(t.get("price_at_entry", 0), 2),
            "sma_side": "above" if t.get("price_at_entry", 0) > t.get("sma_val", 0) and t.get("sma_val", 0) > 0 else "below",
        })
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="GOATv2 Numba Backtest — SMA Filter Study")
    ...
    # ══════════════════════════════════════════════════════════
    # RUN ALL COMBINATIONS: BE level × SMA filter
    # ══════════════════════════════════════════════════════════

    all_results = {}

    for use_sma in [False, True]:
        for be in BE_LEVELS:
            label_be = f"{be}R BE" if be else "No BE"
            label_sma = "SMA200" if use_sma else "No Filter"
            label = f"{label_be} | {label_sma}"

            trades = run_backtest(pre, rr_ratio=rr, be_trigger_r=be,
                                  use_sma_filter=use_sma)
            print_results(trades, label, rr_ratio=rr)
            all_results[(be, use_sma)] = trades

    # Print trade tables for recommended configs only
    print("\n" + "=" * 80)
    print("  DETAILED TRADE TABLES (key strategies only)")
    print("=" * 80)
    for be, use_sma in [(0.5, False), (0.5, True)]:
        label_be = f"{be}R BE"
        label_sma = "SMA200" if use_sma else "No Filter"
        print_trade_table(all_results[(be, use_sma)], f"{label_be} | {label_sma}")
    ...
    print(f"\n{'='*120}")
    print(f"  FULL COMPARISON — {symbol} {timeframe} {days}d RR={rr}")
    print(f"{'='*120}")
    print(f"\n  {'BE Level':<10} {'Filter':<12} {'W':>4} {'L':>4} {'BE':>4} "
          f"{'Total':>5} {'WR%':>6} {'Net R':>8} {'USD':>14} {'MaxDD':>7} "
          f"{'W dur':>6} {'L dur':>6} {'R/trade':>8}")
    print("  " + "-" * 112)

    for use_sma in [False, True]:
        sma_label = "SMA200" if use_sma else "No Filter"
        for be in BE_LEVELS:
            tds = all_results[(be, use_sma)]
            be_label = f"{be}R" if be else "None"
            ...
            avg_w = f"{np.mean(w_durs):.0f}" if w_durs else "—"
            avg_l = f"{np.mean(l_durs):.0f}" if l_durs else "—"
            r_per = pnl / tot if tot > 0 else 0

            print(f"  {be_label:<10} {sma_label:<12} {w:>4} {l:>4} {b:>4} "
                  f"{tot:>5} {wr:>5.1f}% {pnl:>+7.1f}R ${pnl*1000:>+12,.0f} "
                  f"{max_dd:>6.1f}R {avg_w:>6} {avg_l:>6} {r_per:>+7.2f}R")
    ...
    print(f"\n{'='*80}")
    print(f"  SMA200 IMPACT ANALYSIS (difference: SMA - No Filter)")
    print(f"{'='*80}")
    print(f"\n  {'BE Level':<10} {'ΔTrades':>8} {'ΔWins':>6} {'ΔLoss':>6} "
          f"{'ΔNet R':>8} {'ΔUSD':>12} {'ΔMaxDD':>8} {'ΔWR%':>8}")
    print("  " + "-" * 70)

    for be in BE_LEVELS:
        t_no = all_results[(be, False)]
        t_sma = all_results[(be, True)]
        be_label = f"{be}R" if be else "None"
        ...
        def stats(tds):
            ...
            for t in tds:
                if t["pnl_r"] is not None:
                    ...
                    if dd > mdd:
                        mdd = dd
            return w, l, b, tot, wr, pnl, mdd

        w1, l1, b1, tot1, wr1, pnl1, dd1 = stats(t_no)
        w2, l2, b2, tot2, wr2, pnl2, dd2 = stats(t_sma)

        print(f"  {be_label:<10} {tot2-tot1:>+8} {w2-w1:>+6} {l2-l1:>+6} "
              f"{pnl2-pnl1:>+7.1f}R ${(pnl2-pnl1)*1000:>+11,.0f} "
              f"{dd2-dd1:>+7.1f}R {wr2-wr1:>+7.1f}%")

    # ══════════════════════════════════════════════════════════
    # BASELINE SMA ANALYSIS (what % of trades are with/against SMA)
    # ══════════════════════════════════════════════════════════

    baseline = all_results[(None, False)]
    print(f"\n{'='*80}")
    print(f"  BASELINE TRADE SMA ALIGNMENT (No BE, No Filter)")
    print(f"{'='*80}")

    with_sma = []   # BULL above SMA or BEAR below SMA
    against_sma = [] # BULL below SMA or BEAR above SMA
    ...
    for t in baseline:
        ...
        if (t["side"] == "BULL" and pe > sv) or (t["side"] == "BEAR" and pe < sv):
            ...
            against_sma.append(t)

    for label, group in [("WITH SMA (aligned)", with_sma), ("AGAINST SMA (filtered out)", against_sma)]:
        if not group:
            continue
        ...
        print(f"\n  {label}:")
        print(f"    Trades: {tot} | ✅ {w} TP | ❌ {l} SL | 🔄 {b} BE")
        print(f"    Win Rate: {wr:.1f}% | Net: {pnl:+.1f}R (${pnl*1000:+,.0f})")
        if tot > 0:
            print(f"    R per trade: {pnl/tot:+.2f}R")

    # ══════════════════════════════════════════════════════════
    # LOSS MAX-R (baseline)
    # ══════════════════════════════════════════════════════════

    print(f"\n  📐 LOSS MAX-R (baseline, no BE, no filter):")
    baseline_losses = [t for t in baseline if t["result"] == "SL"]
    if baseline_losses:
    ...
    print(f"\n📁 Exporting CSVs...")
    for use_sma in [False, True]:
        sma_tag = "sma" if use_sma else "nofilt"
        for be in BE_LEVELS:
            be_tag = f"be{be}".replace(".", "") if be else "baseline"
            export_csv(all_results[(be, use_sma)],
                       f"goat_sma_{sym_safe}_{timeframe}_{be_tag}_{sma_tag}.csv")

    print(f"\n{'='*60}")
    print(f"  💡 Open {symbol} {timeframe} on TradingView")
    print(f"  💡 Add SMA 200 overlay to see alignment")
    print(f"{'='*60}")