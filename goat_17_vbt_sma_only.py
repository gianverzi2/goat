"""
GOATv2 Numba Backtest — 0.5R BE + 200 SMA Filter + Case Selection
Long only above SMA200, Short only below SMA200.

Usage:
  python3 goat_15_vbt_sma.py --symbol BTC/USDT:USDT --tf 5m --days 180
  python3 goat_15_vbt_sma.py --symbol BTC/USDT:USDT --tf 30m --days 180
  python3 goat_15_vbt_sma.py --symbol BTC/USDT:USDT --tf 5m --days 180 --cases 1
  python3 goat_15_vbt_sma.py --symbol BTC/USDT:USDT --tf 5m --days 180 --cases 13
"""
...
# ═══════════════════════════════════════════════════════════════════
# SECTION 7: SMA CALCULATION
# ═══════════════════════════════════════════════════════════════════

def calculate_sma(raw_close, period=200):
    sma = np.full(len(raw_close), np.nan, dtype=np.float64)
    if len(raw_close) < period:
        return sma
    cs = np.cumsum(raw_close)
...
def precompute_all(df_raw):
    ...
    print("  Computing HA + patterns + pivots + sparse tables + SMA200...")
...
def run_backtest(pre, rr_ratio=3, be_trigger_r=0.5, use_sma_filter=True, warmup=300,
                 enable_c1=True, enable_c2=True, enable_c3=True):
    t0 = time_module.perf_counter()

    cases_str = f"{'C1' if enable_c1 else ''}{'C2' if enable_c2 else ''}{'C3' if enable_c3 else ''}"
    sma_label = "SMA200 FILTER" if use_sma_filter else "NO FILTER"
    print(f"\n{'='*60}")
    print(f"BACKTEST: {be_trigger_r}R BE | {sma_label} | Cases: {cases_str}")
    print(f"{'='*60}")
            ...
            for s in sig_by_bar[bar]:
                ...
                if use_sma_filter:
                    ...
                    if np.isnan(sma_val):
                        filtered_count += 1
                        continue
                    price_now = raw_close[bar]
                    if side_str == "BULL" and price_now < sma_val:
                        filtered_count += 1
                        filtered_bull += 1
                        continue
                    if side_str == "BEAR" and price_now > sma_val:
                        filtered_count += 1
                        filtered_bear += 1
                        continue
                ...
                trade = {
                    ...
                    "sma_val": sma200[bar] if not np.isnan(sma200[bar]) else 0.0,
    ...
    el = time_module.perf_counter() - t0
    if use_sma_filter:
        print(f"    SMA filter removed {filtered_count} signals "
              f"({filtered_bull} BULL below SMA, {filtered_bear} BEAR above SMA)")
...
def print_results(trades, label, rr_ratio=3):
    ...
    if max_dd > 0:
        print(f"📊 Profit Factor: {pnl/max_dd:.2f}x drawdown")
...
def print_trade_table(trades, label=""):
    ...
    print(f"{'#':>3} {'Side':<5} {'Case':<4} {'Res':<3} {'PnL':>6} "
          ...
          f"{'Dur':>4} {'SMA':>5} {'Trigger Time':<22} {'Exit Time':<22}")
    print("-" * 165)
    ...
    for i, t in enumerate(trades):
        ...
        sma_v = t.get("sma_val", 0)
        price_e = t.get("price_at_entry", t["entry"])
        if sma_v > 0:
            sma_rel = "↑" if price_e > sma_v else "↓"
        else:
            sma_rel = "—"
...
def export_csv(trades, filename):
    ...
    for i, t in enumerate(trades):
        sma_v = t.get("sma_val", 0)
        pe = t.get("price_at_entry", t["entry"])
        rows.append({
            ...
            "sma200": round(sma_v, 2),
            "price_at_entry": round(pe, 2),
            "sma_side": "above" if pe > sma_v and sma_v > 0 else "below",
        })
...
def parse_cases(cases_str):
    """Parse --cases argument into (enable_c1, enable_c2, enable_c3)."""
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="GOATv2 — 0.5R BE + SMA200 Filter")
    ...
    parser.add_argument("--cases", type=str, default="123",
                        help="Which cases to enable: 1, 2, 3, 12, 13, 23, 123, or all (default: 123)")
    ...
    print("=" * 60)
    print(f"GOATv2 — 0.5R BE + SMA200 FILTER")
    print(f"{symbol} — {timeframe} — {days}d — RR {rr}")
    print(f"Cases: {cases_str} | Warmup: {warmup} bars")
    print("=" * 60)
    ...
    # ── 1. Without SMA filter (reference) ──
    trades_no_filt, _, _, _ = run_backtest(
        pre, rr_ratio=rr, be_trigger_r=0.5, use_sma_filter=False, warmup=warmup,
        enable_c1=enable_c1, enable_c2=enable_c2, enable_c3=enable_c3)
    print_results(trades_no_filt, f"0.5R BE — NO FILTER | {cases_str} (reference)", rr_ratio=rr)
    print_trade_table(trades_no_filt, f"0.5R BE — NO FILTER | {cases_str}")

    # ── 2. With SMA filter ──
    trades_sma, filt_total, filt_bull, filt_bear = run_backtest(
        pre, rr_ratio=rr, be_trigger_r=0.5, use_sma_filter=True, warmup=warmup,
        enable_c1=enable_c1, enable_c2=enable_c2, enable_c3=enable_c3)
    print_results(trades_sma, f"0.5R BE + SMA200 | {cases_str}", rr_ratio=rr)
    print_trade_table(trades_sma, f"0.5R BE + SMA200 | {cases_str}")
    ...
    print(f"\n{'='*90}")
    print(f"  COMPARISON — {symbol} {timeframe} {days}d RR={rr} | {cases_str}")
    print(f"{'='*90}")

    print(f"\n  {'Strategy':<30} {'W':>4} {'L':>4} {'BE':>4} {'Tot':>5} "
          f"{'WR%':>6} {'Net R':>8} {'USD':>14} {'MaxDD':>7} {'R/trade':>8}")
    print("  " + "-" * 85)

    for name, tds in [("0.5R BE — No Filter", trades_no_filt),
                       ("0.5R BE + SMA200", trades_sma)]:
        ...
        print(f"  {name:<30} {w:>4} {l:>4} {b:>4} {tot:>5} "
              f"{wr:>5.1f}% {pnl:>+7.1f}R ${pnl*1000:>+12,.0f} "
              f"{dd:>6.1f}R {rpt:>+7.2f}R")

    # ── SMA Impact ──
    w1 = len([t for t in trades_no_filt if t["result"] == "TP"])
    l1 = len([t for t in trades_no_filt if t["result"] == "SL"])
    b1 = len([t for t in trades_no_filt if t["result"] == "BE"])
    pnl1 = sum(t["pnl_r"] for t in trades_no_filt if t["pnl_r"] is not None)
    dd1 = calc_max_dd(trades_no_filt)

    w2 = len([t for t in trades_sma if t["result"] == "TP"])
    l2 = len([t for t in trades_sma if t["result"] == "SL"])
    b2 = len([t for t in trades_sma if t["result"] == "BE"])
    pnl2 = sum(t["pnl_r"] for t in trades_sma if t["pnl_r"] is not None)
    dd2 = calc_max_dd(trades_sma)

    print(f"\n  📊 SMA200 FILTER IMPACT:")
    print(f"     Signals filtered: {filt_total} ({filt_bull} BULL below SMA, {filt_bear} BEAR above SMA)")
    print(f"     Trades:  {w1+l1+b1} → {w2+l2+b2} ({w2+l2+b2-(w1+l1+b1):+d})")
    print(f"     Wins:    {w1} → {w2} ({w2-w1:+d})")
    print(f"     Losses:  {l1} → {l2} ({l2-l1:+d})")
    print(f"     BE:      {b1} → {b2} ({b2-b1:+d})")
    print(f"     Net R:   {pnl1:+.1f} → {pnl2:+.1f} ({pnl2-pnl1:+.1f}R)")
    print(f"     MaxDD:   {dd1:.1f}R → {dd2:.1f}R ({dd2-dd1:+.1f}R)")

    # ── What was filtered: analyze baseline trades against SMA ──
    print(f"\n  📊 WHAT THE FILTER REMOVED (from baseline no-filter trades):")
    aligned = []
    against = []
    for t in trades_no_filt:
        if t["result"] == "OPEN":
            continue
        sv = t.get("sma_val", 0)
        pe = t.get("price_at_entry", t["entry"])
        if sv <= 0:
            continue
        if (t["side"] == "BULL" and pe > sv) or (t["side"] == "BEAR" and pe < sv):
            aligned.append(t)
        else:
            against.append(t)

    for label, group in [("KEPT (aligned with SMA)", aligned),
                          ("REMOVED (against SMA)", against)]:
        if not group:
            print(f"\n     {label}: 0 trades")
            continue
    ...
    print(f"\n📁 Exporting CSVs...")
    export_csv(trades_no_filt, f"goat_sma_{sym_safe}_{timeframe}_05be_nofilt_{cases_tag}.csv")
    export_csv(trades_sma, f"goat_sma_{sym_safe}_{timeframe}_05be_sma200_{cases_tag}.csv")

    print(f"\n{'='*60}")
    print(f"  💡 Open {symbol} {timeframe} on TradingView")
    print(f"  💡 Add SMA 200 overlay to validate alignment")
    print(f"{'='*60}")