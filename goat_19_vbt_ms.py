"""
GOATv2 Numba Backtest — 2.0R BE + M30 Market Structure Filter
M30 HH+HL = longs only, M30 LH+LL = shorts only.
Entries always on M5.

Usage:
  python3 goat_16_vbt_ms.py --symbol BTC/USDT:USDT --tf 5m --days 180
  python3 goat_16_vbt_ms.py --symbol BTC/USDT:USDT --tf 5m --days 180 --be 0.5
"""
...
# ═══════════════════════════════════════════════════════════════════
# SECTION 7: M30 MARKET STRUCTURE (HH/HL/LH/LL)
# ═══════════════════════════════════════════════════════════════��═══

MS_PIVOT_LEN = 2  # 2+1+2 = 5 candles on M30

def compute_htf_pivots(df_raw, htf="30m", pivot_len=2):
    """
    Resample M5 to M30, find swing highs/lows on M30,
    determine market structure (HH/HL vs LH/LL),
    map bias back to each M5 bar.

    Returns: ms_bias array (length = M5 bars)
      +1 = bullish (HH+HL), -1 = bearish (LH+LL), 0 = undefined
    """
    df = df_raw.copy()
    ...
    htf_ts = htf_ohlc.index
    n_htf = len(htf_high)

    # Find swing highs and swing lows on M30
    swing_high_idx = []
    swing_high_val = []
    swing_low_idx = []
    swing_low_val = []

    for i in range(pivot_len, n_htf - pivot_len):
        # Swing high
        is_sh = True
        for off in range(1, pivot_len + 1):
            if not (htf_high[i] > htf_high[i - off] and htf_high[i] > htf_high[i + off]):
                is_sh = False
                break
        if is_sh:
            swing_high_idx.append(i)
            swing_high_val.append(htf_high[i])

        # Swing low
        is_sl = True
        for off in range(1, pivot_len + 1):
            if not (htf_low[i] < htf_low[i - off] and htf_low[i] < htf_low[i + off]):
                is_sl = False
                break
        if is_sl:
            swing_low_idx.append(i)
            swing_low_val.append(htf_low[i])

    # Determine bias per M30 bar
    # Walk through time, track last 2 swing highs and last 2 swing lows
    htf_bias = np.zeros(n_htf, dtype=np.int64)

    last_sh_1 = np.nan  # most recent swing high
    last_sh_2 = np.nan  # second most recent
    last_sl_1 = np.nan  # most recent swing low
    last_sl_2 = np.nan  # second most recent

    sh_ptr = 0
    sl_ptr = 0

    for i in range(n_htf):
        # Update swing highs confirmed at this bar
        while sh_ptr < len(swing_high_idx) and swing_high_idx[sh_ptr] + pivot_len <= i:
            last_sh_2 = last_sh_1
            last_sh_1 = swing_high_val[sh_ptr]
            sh_ptr += 1

        # Update swing lows confirmed at this bar
        while sl_ptr < len(swing_low_idx) and swing_low_idx[sl_ptr] + pivot_len <= i:
            last_sl_2 = last_sl_1
            last_sl_1 = swing_low_val[sl_ptr]
            sl_ptr += 1

        # Determine structure
        hh = (not np.isnan(last_sh_1) and not np.isnan(last_sh_2) and last_sh_1 > last_sh_2)
        hl = (not np.isnan(last_sl_1) and not np.isnan(last_sl_2) and last_sl_1 > last_sl_2)
        lh = (not np.isnan(last_sh_1) and not np.isnan(last_sh_2) and last_sh_1 < last_sh_2)
        ll = (not np.isnan(last_sl_1) and not np.isnan(last_sl_2) and last_sl_1 < last_sl_2)

        if hh and hl:
            htf_bias[i] = 1   # Bullish
        elif lh and ll:
            htf_bias[i] = -1  # Bearish
        else:
            # Mixed or not enough data — keep previous bias
            if i > 0:
                htf_bias[i] = htf_bias[i - 1]

    # Map M30 bias back to M5 bars via forward fill
    htf_bias_series = pd.Series(htf_bias, index=htf_ts, name='ms_bias')
    m5_index = pd.to_datetime(df_raw['timestamp'])
    ms_bias_m5 = htf_bias_series.reindex(m5_index, method='ffill').values

    # Fill any NaN at start
    ms_bias_m5 = np.where(np.isnan(ms_bias_m5), 0, ms_bias_m5).astype(np.int64)

    print(f"    M30 pivots: {len(swing_high_idx)} swing highs, {len(swing_low_idx)} swing lows")

    # Count bias distribution
    n_bull = np.sum(ms_bias_m5 == 1)
    n_bear = np.sum(ms_bias_m5 == -1)
    n_neut = np.sum(ms_bias_m5 == 0)
    total = len(ms_bias_m5)
    print(f"    M30 bias on M5: BULL {n_bull}/{total} ({n_bull/total*100:.0f}%) | "
          f"BEAR {n_bear}/{total} ({n_bear/total*100:.0f}%) | "
          f"NEUTRAL {n_neut}/{total} ({n_neut/total*100:.0f}%)")
...
def precompute_all(df_raw):
    ...
    print("  Computing HA + patterns + pivots + sparse tables + M30 structure...")
    ...
    print("  Computing M30 market structure (HH/HL/LH/LL)...")
    ms_bias = compute_htf_pivots(df_raw, htf="30m", pivot_len=MS_PIVOT_LEN)

    el = time_module.perf_counter() - t0
    print(f"    {len(piv_low_idx)} pivot lows, {len(piv_high_idx)} pivot highs (M5 HA)")
...
def run_backtest(pre, rr_ratio=3, be_trigger_r=2.0,
                 use_ms_filter=False, warmup=300):
    t0 = time_module.perf_counter()

    filt_label = "M30 HH/HL FILTER" if use_ms_filter else "NO FILTER"
    print(f"\n{'='*60}")
    ...
    for bar in range(warmup, n):
        ...
        if bar in sig_by_bar:
            for s in sig_by_bar[bar]:
                ...
                # ── M30 MARKET STRUCTURE FILTER ──
                bias = ms_bias[bar]
                ms_label = "BULL" if bias == 1 else "BEAR" if bias == -1 else "NEUT"

                if use_ms_filter:
                    if bias == 0:
                        # Neutral — no clear structure, skip
                        filtered_count += 1
                        filtered_neutral += 1
                        continue
                    if side_str == "BULL" and bias == -1:
                        # Long in bearish structure, skip
                        filtered_count += 1
                        filtered_bull += 1
                        continue
                    if side_str == "BEAR" and bias == 1:
                        # Short in bullish structure, skip
                        filtered_count += 1
                        filtered_bear += 1
                        continue
    ...
    el = time_module.perf_counter() - t0
    if use_ms_filter:
        print(f"    MS filter removed {filtered_count} signals "
              f"({filtered_bull} BULL in bear MS, {filtered_bear} BEAR in bull MS, "
              f"{filtered_neutral} in neutral)")
    print(f"  ✅ Done in {el:.1f}s — {len(trades)} trades")
    return trades, filtered_count, filtered_bull, filtered_bear, filtered_neutral
...
def print_results(trades, label, rr_ratio=3):
    ...
    # BY M30 BIAS
    print(f"\n📊 BY M30 STRUCTURE:")
    for bias_val, bias_name in [(1, "BULL MS"), (-1, "BEAR MS"), (0, "NEUTRAL")]:
        group = [t for t in trades if t.get("ms_bias") == bias_val and t["result"] in ("TP", "SL", "BE")]
        if not group:
            continue
        ...
        tot = w + l + b
        gpnl = sum(t["pnl_r"] for t in group if t["pnl_r"] is not None)
        print(f"   {bias_name}: {w}W/{l}L/{b}BE = {w/tot*100:.0f}% | Net: {gpnl:+.1f}R")
...
def print_trade_table(trades, label=""):
    ...
    print(f"{'#':>3} {'Side':<5} {'Case':<4} {'Res':<3} {'PnL':>6} "
          ...
          f"{'Dur':>4} {'M30':>5} {'Trigger Time':<22} {'Exit Time':<22}")
    print("-" * 170)
...
def export_csv(trades, filename):
    ...
    for i, t in enumerate(trades):
        rows.append({
            ...
            "m30_bias": t.get("ms_bias", 0),
            "m30_label": t.get("ms_label", ""),
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="GOATv2 — BE + M30 Market Structure Filter")
    ...
    print("=" * 60)
    print(f"GOATv2 — {be}R BE + M30 MARKET STRUCTURE FILTER")
    print(f"{symbol} — {timeframe} — {days}d — RR {rr} — BE {be}R")
    print(f"M30 Structure: HH+HL = longs only, LH+LL = shorts only")
    print(f"M30 Pivot: {MS_PIVOT_LEN}+1+{MS_PIVOT_LEN} = {MS_PIVOT_LEN*2+1} candles")
    print("=" * 60)
    ...
    trades_ref, _, _, _, _ = run_backtest(
        pre, rr_ratio=rr, be_trigger_r=be, use_ms_filter=False)
    print_results(trades_ref, f"{be}R BE — NO FILTER (reference)", rr_ratio=rr)
    print_trade_table(trades_ref, f"{be}R BE — NO FILTER")

    # ── 2. M30 Market Structure filter ──
    trades_ms, filt_tot, filt_bull, filt_bear, filt_neut = run_backtest(
        pre, rr_ratio=rr, be_trigger_r=be, use_ms_filter=True)
    print_results(trades_ms, f"{be}R BE + M30 HH/HL FILTER", rr_ratio=rr)
    print_trade_table(trades_ms, f"{be}R BE + M30 HH/HL")
    ...
    print(f"\n{'='*100}")
    print(f"  COMPARISON — {symbol} {timeframe} {days}d RR={rr} | {be}R BE")
    print(f"{'='*100}")
    ...
    for name, tds in [("No Filter (ref)", trades_ref),
                       ("M30 HH/HL Structure", trades_ms)]:
    ...
    w1 = len([t for t in trades_ms if t["result"] == "TP"])
    l1 = len([t for t in trades_ms if t["result"] == "SL"])
    b1 = len([t for t in trades_ms if t["result"] == "BE"])
    pnl1 = sum(t["pnl_r"] for t in trades_ms if t["pnl_r"] is not None)
    dd1 = calc_max_dd(trades_ms)

    print(f"\n  📊 M30 STRUCTURE FILTER IMPACT:")
    print(f"     Signals filtered: {filt_tot} "
          f"({filt_bull} BULL in bear MS, {filt_bear} BEAR in bull MS, "
          f"{filt_neut} in neutral)")
    print(f"     Trades:  {w0+l0+b0} → {w1+l1+b1} ({w1+l1+b1-(w0+l0+b0):+d})")
    print(f"     Wins:    {w0} → {w1} ({w1-w0:+d})")
    print(f"     Losses:  {l0} → {l1} ({l1-l0:+d})")
    print(f"     BE:      {b0} → {b1} ({b1-b0:+d})")
    print(f"     Net R:   {pnl0:+.1f} → {pnl1:+.1f} ({pnl1-pnl0:+.1f}R)")
    print(f"     MaxDD:   {dd0:.1f}R → {dd1:.1f}R ({dd1-dd0:+.1f}R)")

    # ── What was filtered ──
    print(f"\n{'='*80}")
    print(f"  WHAT THE FILTER REMOVES (from no-filter baseline)")
    print(f"{'='*80}")
    ...
    for t in trades_ref:
        if t["result"] == "OPEN":
            continue
        bias = t.get("ms_bias", 0)
        if bias == 0:
            neutral.append(t)
        elif (t["side"] == "BULL" and bias == 1) or (t["side"] == "BEAR" and bias == -1):
            aligned.append(t)
        else:
            against.append(t)

    for label, group in [("KEPT (aligned with M30 structure)", aligned),
                          ("REMOVED (against M30 structure)", against),
                          ("REMOVED (neutral / no structure)", neutral)]:
        if not group:
            print(f"\n     {label}: 0 trades")
            continue
        w = len([t for t in group if t["result"] == "TP"])
    ...
    export_csv(trades_ref, f"goat_ms_{sym_safe}_{timeframe}_{be_tag}_nofilt.csv")
    export_csv(trades_ms, f"goat_ms_{sym_safe}_{timeframe}_{be_tag}_m30ms.csv")

    print(f"\n{'='*60}")
    print(f"  💡 Open {symbol} {timeframe} on TradingView")
    print(f"  💡 Check M30 swing structure to validate HH/HL/LH/LL")
    print(f"{'='*60}")