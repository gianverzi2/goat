...
Usage:
  ...
  # 50% off at 1.5R (default) — lock +0.75R
    python3 goat_20_vbt_equity.py --symbol SOL/USDT:USDT --tf 5m --days 180 --partial 1.5

    # 33% off at 1.5R — lock +0.50R, 67% rides
    python3 goat_20_vbt_equity.py --symbol SOL/USDT:USDT --tf 5m --days 180 --partial 1.5 --partial-pct 33
...
def check_case1_jit(ha_close, ha_high, ha_low,
    ...
    for pi in range(n_valid):
        ...
        for k in range(prior_idx + 1, cur + 1):
...
def check_case2_jit(ha_close, ha_high, ha_low,
    ...
    for k in range(best_idx + 1, cur + 1):
        if is_bear:
            wick = ha_high[k]
            if wick < line_level:
                continue
...
def check_case3_jit(ha_close, ha_high, ha_low,
    ...
    for k in range(piv_idx + 1, cur + 1):
        if is_bear:
            wick = ha_high[k]
