"""GOATv2 Scanner — Hyperliquid Daily Timeframe"""
...
CFG = {
    ...
    "max_symbols": 0,
    "fetch_limit": 200,
    "post_close_delay_sec": 10,
    "max_retries": 3,
    "rate_limit_delay": 0.2,
    "rr_ratio": 3,
    "pivot_length": 2,
    "reload_every_n_cycles": 7,           # weekly reload
    "enable_provisional": True,
    "provisional_offset_sec": 3600,       # 1h before close
...
def compute_scan_plan(now: datetime, tf_minutes: int, post_close_delay_sec: int):
    """
    Return a list of (run_at, run_type) for the current daily bar:
      - provisional: 1h before 00:00 UTC close (if enabled)
      - final: a few seconds after 00:00 UTC close
    Daily bars close at 00:00 UTC.
    """
    # ── Next daily close is the next 00:00 UTC ───────────────────
    today_close = now.replace(hour=0, minute=0, second=0, microsecond=0)
    if now >= today_close:
    ...
    # ── Provisional: 1h before close ─────────────────────────────
    if CFG.get("enable_provisional", False):
        offset = CFG.get("provisional_offset_sec", 3600)
    ...
    try:
        ...
        while True:
            ...
            for run_at, run_type in plan:
                ...
                if sleep_s > 0:
                    logging.info(
                        f"Next {run_type} scan at {run_at.strftime('%Y-%m-%d %H:%M:%S')} UTC "
